...
static void _drawPixel(int16_t x, int16_t y, color_t color, uint8_t sel)
{
	...
	if ((x < tft_dispWin.x1) || (y < tft_dispWin.y1) || (x > tft_dispWin.x2) || (y > tft_dispWin.y2))
		return;
...
{
	// clipping
	if ((x < tft_dispWin.x1) || (x > tft_dispWin.x2) || (y > tft_dispWin.y2))
		return;
	if (y < tft_dispWin.y1)
	{
		h -= (tft_dispWin.y1 - y);
	...
	if (h < 0)
		h = 0;
	if ((y + h) > (tft_dispWin.y2 + 1))
		h = tft_dispWin.y2 - y + 1;
	if (h == 0)
		h = 1;
...
{
	// clipping
	if ((y < tft_dispWin.y1) || (x > tft_dispWin.x2) || (y > tft_dispWin.y2))
		return;
	if (x < tft_dispWin.x1)
	{
		w -= (tft_dispWin.x1 - x);
	...
	if (w < 0)
		w = 0;
	if ((x + w) > (tft_dispWin.x2 + 1))
		w = tft_dispWin.x2 - x + 1;
	if (w == 0)
		w = 1;
...
{
	// clipping
	if ((x >= tft_dispWin.x2) || (y > tft_dispWin.y2))
		return;
...
static void getMaxWidthHeight()
{
	...
	while (cc != 0xFF)
	{
		...
		cy += ch;
		if (cw > tft_cfont.max_x_size)
			tft_cfont.max_x_size = cw;
		if (cd > tft_cfont.max_x_size)
			tft_cfont.max_x_size = cd;
		if (ch > tft_cfont.y_size)
			tft_cfont.y_size = ch;
		if (cy > tft_cfont.y_size)
			tft_cfont.y_size = cy;
		if (cw != 0)
		{
			// packed bits
	...
	else
	{
		if (font == USER_FONT)
		{
			if (load_file_font(font_file, 0) != 0)
				tft_cfont.font = tft_DefaultFont;
			else
				tft_cfont.font = userfont;
		}
		else if (font == DEJAVU18_FONT)
			tft_cfont.font = tft_Dejavu18;
		else if (font == DEJAVU24_FONT)
			tft_cfont.font = tft_Dejavu24;
		else if (font == UBUNTU16_FONT)
			tft_cfont.font = tft_Ubuntu16;
		else if (font == COMIC24_FONT)
			tft_cfont.font = tft_Comic24;
		else if (font == MINYA24_FONT)
			tft_cfont.font = tft_minya24;
		else if (font == TOONEY32_FONT)
			tft_cfont.font = tft_tooney32;
		else if (font == SMALL_FONT)
			tft_cfont.font = tft_SmallFont;
		else if (font == DEF_SMALL_FONT)
			tft_cfont.font = tft_def_small;
		else
			tft_cfont.font = tft_DefaultFont;
...
static int printProportionalChar(int x, int y)
{
	...
	if (!tft_font_transparent)
		_fillRect(x, y, char_width + 1, tft_cfont.y_size, tft_bg);
...
static void printChar(uint8_t c, int x, int y)
{
		...
		if (color_line)
		{
			...
			for (j = 0; j < tft_cfont.y_size; j++)
			{
				for (k = 0; k < fz; k++)
				{
					...
					{
						if ((ch & mask) != 0)
							color_line[(j * tft_cfont.x_size) + (i + (k * 8))] = tft_fg;
						mask >>= 1;
					}
				}
				temp += (fz);
	...
	if (!tft_font_transparent)
		_fillRect(x, y, tft_cfont.x_size, tft_cfont.y_size, tft_bg);
...
static int rotatePropChar(int x, int y, int offset)
{
	...
	for (int j = 0; j < fontChar.height; j++)
	{
		for (int i = 0; i < fontChar.width; i++)
		{
			...
			if ((ch & mask) != 0)
				_drawPixel(newX, newY, tft_fg, 0);
			else if (!tft_font_transparent)
				_drawPixel(newX, newY, tft_bg, 0);
...
int TFT_getStringWidth(const char *str)
{
	...
	else if (tft_cfont.x_size != 0)
		strWidth = strlen(str) * tft_cfont.x_size; // fixed width font
	else
	{
...
static void _draw7seg(int16_t x, int16_t y, int8_t num, int16_t w, int16_t l, color_t color)
{
	...
	if (!(c & 0x001))
		barVert(x + d, y + d, w, l, tft_bg, tft_bg);
	if (!(c & 0x002))
		barVert(x, y + d, w, l, tft_bg, tft_bg);
	if (!(c & 0x004))
		barVert(x + d, y, w, l, tft_bg, tft_bg);
	if (!(c & 0x008))
		barVert(x, y, w, l, tft_bg, tft_bg);
	if (!(c & 0x010))
		barHor(x, y + 2 * d, w, l, tft_bg, tft_bg);
	if (!(c & 0x020))
		barHor(x, y + d, w, l, tft_bg, tft_bg);
	if (!(c & 0x040))
		barHor(x, y, w, l, tft_bg, tft_bg);
	...
	if (!(c & 0x080))
	{
		...
		if (tft_cfont.offset)
			_drawRect(x + (d / 2), y + 2 * d, 2 * w + 1, 2 * w + 1, tft_bg);
	}
	if (!(c & 0x100))
	{
		// down middle point
		...
		if (tft_cfont.offset)
			_drawRect(x + (d / 2), y + d + 2 * w + 1, 2 * w + 1, l / 2, tft_bg);
	}
	if (!(c & 0x800))
	{
		// up middle point
		...
		if (tft_cfont.offset)
			_drawRect(x + (d / 2), y + (2 * w) + 1 + (l / 2), 2 * w + 1, l / 2, tft_bg);
	}
	if (!(c & 0x200))
	{
		// middle, minus
		...
		if (tft_cfont.offset)
			_drawRect(x + 2 * w + 1, y + d, l, 2 * w + 1, tft_bg);
	}

	// === Draw used segments ===
	if (c & 0x001)
		barVert(x + d, y + d, w, l, color, tft_cfont.color); // down right
	if (c & 0x002)
		barVert(x, y + d, w, l, color, tft_cfont.color); // down left
	if (c & 0x004)
		barVert(x + d, y, w, l, color, tft_cfont.color); // up right
	if (c & 0x008)
		barVert(x, y, w, l, color, tft_cfont.color); // up left
	if (c & 0x010)
		barHor(x, y + 2 * d, w, l, color, tft_cfont.color); // down
	if (c & 0x020)
		barHor(x, y + d, w, l, color, tft_cfont.color); // middle
	if (c & 0x040)
		barHor(x, y, w, l, color, tft_cfont.color); // up
	...
	if (c & 0x080)
	{
		...
		if (tft_cfont.offset)
			_drawRect(x + (d / 2), y + 2 * d, 2 * w + 1, 2 * w + 1, tft_cfont.color);
	}
	if (c & 0x100)
	{
		// down middle point
		...
		if (tft_cfont.offset)
			_drawRect(x + (d / 2), y + d + 2 * w + 1, 2 * w + 1, l / 2, tft_cfont.color);
	}
	if (c & 0x800)
	{
		// up middle point
		...
		if (tft_cfont.offset)
			_drawRect(x + (d / 2), y + (2 * w) + 1 + (l / 2), 2 * w + 1, l / 2, tft_cfont.color);
	}
	if (c & 0x200)
	{
		// middle, minus
		...
		if (tft_cfont.offset)
			_drawRect(x + 2 * w + 1, y + d, l, 2 * w + 1, tft_cfont.color);
	}
}
//==============================================================================
...
void TFT_print(const char *st, int x, int y)
{
	...
	if (tft_cfont.bitmap == 0)
		return; // wrong font selected

	// ** Rotated strings cannot be aligned
	if ((tft_font_rotate != 0) && ((x <= CENTER) || (y <= CENTER)))
		return;

	if ((x < LASTX) || (tft_font_rotate == 0))
		TFT_OFFSET = 0;
	...
	if (x == RIGHT)
		x = tft_dispWin.x2 - tmpw + tft_dispWin.x1;
	else if (x == CENTER)
		x = (((tft_dispWin.x2 - tft_dispWin.x1 + 1) - tmpw) / 2) + tft_dispWin.x1;

	if (y == BOTTOM)
		y = tft_dispWin.y2 - fh + tft_dispWin.y1;
	else if (y == CENTER)
		y = (((tft_dispWin.y2 - tft_dispWin.y1 + 1) - (fh / 2)) / 2) + tft_dispWin.y1;
	...
	if ((tft_y + tmph - 1) > tft_dispWin.y2)
		return;
	...
	for (i = 0; i < stl; i++)
	{
		...
		if (ch == 0x0D)
		{ // === '\r', erase to eol ====
			if ((!tft_font_transparent) && (tft_font_rotate == 0))
				_fillRect(tft_x, tft_y, tft_dispWin.x2 + 1 - tft_x, tmph, tft_bg);
		}
		...
		{ // ==== other characters ====
			...
			else
			{
				...
				{
					// == fixed font
					if ((ch < tft_cfont.offset) || ((ch - tft_cfont.offset) > tft_cfont.numchars))
						ch = tft_cfont.offset;
					if (tft_font_rotate == 0)
					{
...
void TFT_setclipwin(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2)
{
	...
	if (tft_dispWin.x2 >= tft_width + TFT_STATIC_X_OFFSET)
		tft_dispWin.x2 = tft_width + TFT_STATIC_X_OFFSET - 1;
	if (tft_dispWin.y2 >= tft_height + TFT_STATIC_Y_OFFSET)
		tft_dispWin.y2 = tft_height + TFT_STATIC_Y_OFFSET - 1;
	if (tft_dispWin.x1 > tft_dispWin.x2)
		tft_dispWin.x1 = tft_dispWin.x2;
	if (tft_dispWin.y1 > tft_dispWin.y2)
...
int TFT_getfontsize(int *width, int *height)
{
	...
	{
		if (tft_cfont.x_size != 0)
			*width = tft_cfont.x_size; // fixed width font
		else
			*width = tft_cfont.max_x_size; // proportional font
...
{
	if (tft_cfont.bitmap == 1)
		return tft_cfont.y_size; // Bitmap font
	else if (tft_cfont.bitmap == 2)
...
)
{
	...
	if ((left > tft_dispWin.x2) || (top > tft_dispWin.y2))
		return 1; // out of screen area, return
	if ((right < tft_dispWin.x1) || (bottom < tft_dispWin.y1))
		return 1; // out of screen area, return

	if (left < tft_dispWin.x1)
		dleft = tft_dispWin.x1;
	else
		dleft = left;
	if (top < tft_dispWin.y1)
		dtop = tft_dispWin.y1;
	else
		dtop = top;
	if (right > tft_dispWin.x2)
		dright = tft_dispWin.x2;
	else
		dright = right;
	if (bottom > tft_dispWin.y2)
		dbottom = tft_dispWin.y2;
	else
		dbottom = bottom;

	if ((dleft > tft_dispWin.x2) || (dtop > tft_dispWin.y2))
		return 1; // out of screen area, return
	if ((dright < tft_dispWin.x1) || (dbottom < tft_dispWin.y1))
		return 1; // out of screen area, return
...
void TFT_jpg_image(int x, int y, uint8_t scale, const char *fname, uint8_t *buf, int size)
{
	...
	if (work)
	{
		...
		{
			if (x == CENTER)
				x = ((tft_dispWin.x2 - tft_dispWin.x1 + 1 - (int)(jd.width >> scale)) / 2) + tft_dispWin.x1;
			else if (x == RIGHT)
				x = tft_dispWin.x2 + 1 - (int)(jd.width >> scale);

			if (y == CENTER)
				y = ((tft_dispWin.y2 - tft_dispWin.y1 + 1 - (int)(jd.height >> scale)) / 2) + tft_dispWin.y1;
			else if (y == BOTTOM)
				y = tft_dispWin.y2 + 1 - (int)(jd.height >> scale);

			if (x < ((tft_dispWin.x2 - 1) * -1))
				x = (tft_dispWin.x2 - 1) * -1;
			if (y < ((tft_dispWin.y2 - 1)) * -1)
				y = (tft_dispWin.y2 - 1) * -1;
			if (x > (tft_dispWin.x2 - 1))
				x = tft_dispWin.x2 - 1;
			if (y > (tft_dispWin.y2 - 1))
				y = tft_dispWin.y2 - 1;
...
int TFT_bmp_image(int x, int y, uint8_t scale, const char *fname, uint8_t *imgbuf, int size)
{
	...
	if (x == CENTER)
		x = ((tft_dispWin.x2 - tft_dispWin.x1 + 1 - img_xlen) / 2) + tft_dispWin.x1;
	else if (x == RIGHT)
		x = tft_dispWin.x2 + 1 - img_xlen;

	if (y == CENTER)
		y = ((tft_dispWin.y2 - tft_dispWin.y1 + 1 - img_ylen) / 2) + tft_dispWin.y1;
	else if (y == BOTTOM)
		y = tft_dispWin.y2 + 1 - img_ylen;
...
int TFT_read_touch(int *x, int *y, uint8_t raw)
{
	...
	*y = 0;
	...
	ESP_LOGW("TCH", "Result %d -> %d : %d", result, X, Y);
