...
void INA219::begin(i2c_port_t v_i2c_num, gpio_num_t sda_io_num, gpio_num_t scl_io_num)
{
  ...
  Rv = i2c_driver_install(i2c_num, conf.mode, I2C_MASTER_RX_BUF_DISABLE, I2C_MASTER_TX_BUF_DISABLE, 0);

  ESP_LOGE(TAG, "i2c_driver_install returned: %d", Rv);
}
...
{
#if (INA219_DEBUG == 1)
  Serial.print("Reconfigure with Config: 0x");
  Serial.println(config, HEX);
#endif
  write16(CONFIG_R, config);
}
...
{
#if (INA219_DEBUG == 1)
  Serial.print("Recalibrate with cal: ");
  Serial.println(cal);
#endif
  write16(CAL_R, cal);
}
...
int16_t INA219::read16(t_reg a) const
{
  ...
  ESP_ERROR_CHECK(i2c_master_read_byte(cmd, &cmddata[0], I2C_MASTER_ACK));
  ESP_ERROR_CHECK(i2c_master_read_byte(cmd, &cmddata[1], I2C_MASTER_NACK));
  ESP_ERROR_CHECK(i2c_master_stop(cmd));
  ESP_ERROR_CHECK(i2c_master_cmd_begin(i2c_num, cmd, 1000 / portTICK_RATE_MS));
  i2c_cmd_link_delete(cmd);
